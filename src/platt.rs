...
#[inline(never)]
fn calc_delta_f64(x: u64, eps: f64, λ: f64, x1: u64, x2: u64) -> f64 {
    let mut ret = 0.0;
...
impl<T: MyReal> Platt<T> {
    ...
    #[inline]
    fn Φ(&self, p: T, eps: f64) -> T { (p / T::SQRT_2()).erfc(eps) / 2.0f64 }

    #[inline]
    fn phi(&self, u: T, x: T, eps: f64) -> T { self.Φ((u / x).ln() / self.λ, eps) }
    ...
    pub fn plan(&mut self, x: f64, hints: PlattHints) {
        ...
        let (x1, x2) = self.plan_delta_bounds(λ, x, 0.24);
    ...
    fn plan_delta_bounds(&mut self, λ: f64, x: f64, eps: f64) -> (u64, u64) {
        let eps = eps / 2.0;

        // change of variable: t = ln(u/x) / λ <=> u = x e^(t λ).
        let Ep = |t: f64| x * ((λ * λ / 2.0).exp() * Φ(t - λ) - (t * λ).exp() * Φ(t));
        let Em = |t: f64| x * ((t * λ).exp() * Φ(-t) - (λ * λ / 2.0).exp() * Φ(λ - t));
    ...
    pub fn compute(&mut self, n: u64, hints: PlattHints) -> u64 {
        ...
        let delta = calc_delta_f64(n, 0.5, self.λ.unchecked_cast(), self.x1, self.x2);
        info!("delta = {}", delta);
