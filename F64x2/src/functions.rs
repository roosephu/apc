...
mod tests {
    ...
    fn test_log() {
        ...
        assert_close(x, gt, 0.0, 1e-30);
    }

    #[test]
    fn test_sqrt() {
        let s = f64x2::new(123.0, 0.0);
        assert_close(s.sqrt(), f64x2::new(11.090536506409418, -5.209651269937913e-16), 0.0, 1e-30);
    }

    #[test]
    fn test_exp() {
        let s = f64x2::new(3.4538776394910684, 0.0000000000000001184757763427252);
        assert_close(s.exp(), f64x2::new(31.622776601683793, 7.566535620287156e-16), 0.0, 1e-30);
    }
    ...
    fn test_cos_sin() {
        ...
        assert_close(t.cos(), f64x2::new(0.5107818439368557, 1.0134834604058354e-17), 0.0, 3e-32);

        let s = f64x2::new(23.025850929940457, -0.00000000000000039439938398199903);
        assert_close(s.cos(), f64x2::new(-0.5107818439368557, -1.0134834604058354e-17), 0.0, 3e-32);
        assert_close(s.sin(), f64x2::new(-0.8597103627992777, 4.575936712504712e-17), 0.0, 3e-32);
    }
